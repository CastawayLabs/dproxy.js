...
ProxySocket.prototype.onData = function (data) {
	...
	if (!self.target) {
		...
		self.proxy.getDomainDetails(self.hostname, function (err, details, member) {
			...
			// The proxy should redirect to https if the request is insecure and ssl == true && ssl_only == true
			// The proxy should redirect to http if the request is secure and ssl == false

			// Create the proxy
			self.target = net.createConnection(member.port, member.host);
