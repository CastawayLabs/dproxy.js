...
ProxySocket.prototype.onData = function (data) {
	...
	// The problem with could be that the host header is in another chunk..
	if (!this.hostname) {
...
ProxySocket.prototype.onClose = function () {
	...
	var m = {};
	m['proxy.unique_ips.' + this.hostname] = ip + '|s';
	m['proxy.ips.' + this.socket.remoteAddress] = '1|c';
	m['proxy.finished.time.' + this.hostname] = ms + '|ms';
	m['proxy.finished.length.read.' + this.hostname] = this.socket.bytesRead + '|ms';
	m['proxy.finished.length.write.' + this.hostname] = this.socket.bytesWritten + '|ms';
